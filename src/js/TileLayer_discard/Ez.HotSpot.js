...
Ez.TileLayer.HotSpot.prototype.onAdd = function(map) {
	...
	map.addMapEventListener(Ez.Event.MAP_MOUSEMOVE,this.onMouseMove_);
...
Ez.TileLayer.HotSpot.prototype.onRemove = function(map) {
	...
	layerGroups.forEach(function(layer){
		if (layer.get('layerType') === 'hotspotLayer') {
			map.removeLayer(layer);
		}
	});
	map.un(Ez.Event.MAP_MOUSEMOVE,this.onMouseMove_);
		...
		if (bottomRight in markerData_[zoom]) {
			...
			for (var i = 0; i < len; i++) {
				...
				if (hotspotbounds.containsPoint(mouseCoordinate)) {
					...
					tileLayer.marker_.set('data',data);
...
// 以下为需要保留的类名以及方法
goog.exportSymbol('EzHotSpot',Ez.TileLayer.HotSpot);