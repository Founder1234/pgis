 ...
 */
Ez.HeatMap = function(configOpts) {
...
goog.inherits(Ez.HeatMap, ol.layer.Heatmap);

/**
 * 增加到地图对象中
 * @param  {EzMap} map - 地图对象
 */
Ez.HeatMap.prototype.onAdd = function(map) {
 ...
 * @param  {EzMap} map - 地图对象
 */
Ez.HeatMap.prototype.onRemove = function(map) {
    this.map_ = null;
 ...
 * 显示热力图
 */
Ez.HeatMap.prototype.show = function() {
    this.setVisible(true);
};

/**
 * 隐藏热力图,但是保留在内存中，可以通过{@link Ez.HeatMap#show}方法来显示
 */
Ez.HeatMap.prototype.hide = function() {
 ...
 */
Ez.HeatMap.prototype.addData = function(data) {
...
// 以下为需要保留的类名以及方法
goog.exportSymbol('EzHeatMap', Ez.HeatMap);
