...
|@author: qianleyi
|@date: 2016-10-13
|@descript: 支持EzMarker的聚类
|------------------------------------------------------------------------------
*/
goog.provide('ol.source.Cluster2');
 ...
 * @api
 */
ol.source.Cluster2 = function(options) {
    goog.base(this, {
    ...
    this.source_.on(ol.events.EventType.CHANGE,
        ol.source.Cluster2.prototype.onSourceChange_, this);
};
goog.inherits(ol.source.Cluster2, ol.source.Vector);
 ...
 * @api
 */
ol.source.Cluster2.prototype.getSource = function() {
    return this.source_;
};
 ...
 * @inheritDoc
 */
ol.source.Cluster2.prototype.loadFeatures = function(extent, resolution,
    projection) {
 ...
 * @private
 */
ol.source.Cluster2.prototype.onSourceChange_ = function() {
    this.clear();
    this.cluster_();
    this.addFeatures(this.features_);
    this.changed();
 ...
 * @private
 */
ol.source.Cluster2.prototype.cluster_ = function() {
 ...
 * @private
 */
ol.source.Cluster2.prototype.createCluster_ = function(features) {
    var centroid = [0, 0];
    ...
    if (features.length > 1) {
        cluster = new ol.Feature(new ol.geom.Point(centroid));
        cluster.set('features', features);
    } else {
        cluster = features[0];
    }
